...
# We will also check Quadratic Weighted Kappa Score  
def check_accuracy(loader, model, device="cuda"):
    ...
    for x, y, filename in tqdm(loader):
        ...
        with torch.no_grad():
            predictions = model(x)
           # print("pred yb models",predictions)
        # Convert MSE floats to integer predictions
        predictions[predictions < 0.5] = 0
        predictions[(predictions >= 0.5) & (predictions < 1.5)] = 1
        predictions[(predictions >= 1.5) & (predictions < 2.5)] = 2
        ...
        #predictions = torch.round(predictions).long().view(-1)
        
        predictions = predictions.long().view(-1)
        #print("After casting",predictions)
        
        y = y.view(-1)
      # print("Predictionssss",predictions.shape)
    ...
    all_preds = np.concatenate(all_preds, axis=0, dtype=np.int64).ravel()
    all_labels = np.concatenate(all_labels, axis=0, dtype=np.int64).ravel()

    print("Shapes - all_preds:", all_preds.shape, "all_labels:", all_labels.shape)
    print("DataType - all_preds:", all_preds.dtype, "all_labels:", all_labels.dtype)
